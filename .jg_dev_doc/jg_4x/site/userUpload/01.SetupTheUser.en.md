...
Old: The user needs to be logged in, own a category/gallery and is assigned to a user group with certain rights (However, it could be reactivated.)
...
It shall look like below. In the future, not all green settings should be necessary.
...
May be reactivated:  
~~The user needs an initial category/gallery to build on.~~  
~~1)  Create the category~~  
~~2) Assign it to the user: Edit the category -> Tab "Publishing" -> property owner -> click ~~
 on icon on the right side~~
