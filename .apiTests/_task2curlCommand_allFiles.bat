@echo Off
REM ----------------------------------------
REM Transform all *.tsk files to curl command batch file
...
ECHO "for %%i in (*.tsk) do CALL :task2curlCommand %%i"
for %%i in (*.tsk) do CALL :task2curlCommand %%i
...
ECHO task2curlCommand: "%1"

REM if not exist %1\nul goto :EOF

SET NO_PAUSE=true
CALL _task2curlCommand.bat %1 %dstPath%
SET NO_PAUSE=
